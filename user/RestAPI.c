...
=======
// ip/eport/value
>>>>>>> d63dbd4f9ff64574dd70e2903978b0d85d95db9f
...
//if more than 20 commands change server.c code !!!
};
...
#else
uint32 portsBits[NUM_OF_PORTS] = {BIT0,BIT1,BIT15,BIT16,BIT7};
int PortPinNumber[NUM_OF_PORTS] = {0,1,15,16,7};
#endif
    ...
    if (getValue(tmp, conn->url,'/',i)!= -1)
    {
    	...
    	if (getValue(tmp, conn->url,'/',i)!= -1)
    	{
			...
			if (val == 0)
			{
				...
				os_printf("ePort clear bit %d", bit);
			}
			else
			{
				BIT_SET(flashData->ePort,bit);
				os_printf("ePort set bit %d", bit);
			}
